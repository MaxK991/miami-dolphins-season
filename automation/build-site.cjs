...
const C=require('../core.js'),Feed=require('./calendar-feed.cjs');
...
async function buildSite({root=ROOT,output=path.join(ROOT,'_site'),now=Date.now(),siteBase=process.env.SITE_BASE_URL,automatic=process.env.GITHUB_ACTIONS==='true',fetchJSON=json,previous}={}){
 ...
 // Nothing is published until every data source is validated. A failed job leaves the deployed site intact.
 await fs.mkdir(path.join(output,'calendar'),{recursive:true});
 const files=['index.html','tabelle.html','playoffs.html','app.js','core.js','postseason.js','calendar.js','styles.css','manifest.webmanifest','apple-touch-icon.png','favicon.png','README.md','PRUEFUNG.md','ABO_EINRICHTEN.md'];
