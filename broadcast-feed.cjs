...
function parseRTL(html, source, games, now) {
  ...
  const rows = [...html.matchAll(/<li\b[^>]*>([\s\S]*?)<\/li>/gi)]
    .map(m => text(m[1]).split('|').map(s => s.trim()))
    // RTL sometimes separates weekday and date into two columns.
    .map(a => a.length === 5 && /^(Montag|Dienstag|Mittwoch|Donnerstag|Freitag|Samstag|Sonntag)$/i.test(a[0]) ? [a[0] + ', ' + a[1], ...a.slice(2)] : a)
    .filter(a => a.length === 4 && a[3].includes('@'));
